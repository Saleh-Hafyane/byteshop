...
export class CheckoutComponent implements OnInit{
  ...
  cities:City[]=[]
  constructor(private formService:FormService,private cartService:CartService,private router:Router,private checkoutService:CheckoutService) {
  }
  ngOnInit() {
    this.checkoutFormGroup = new FormGroup({
      ...
      address:  new FormGroup({
        ...
        address: new FormControl("",[Validators.required,Validators.minLength(2),CustomValidators.notOnlySpaces])
      }),
      creditCard:  new FormGroup({
        cardType: new FormControl("",[Validators.required]),
        nameOnCard: new FormControl("",[Validators.required,Validators.minLength(2),CustomValidators.notOnlySpaces]),
        cardNumber: new FormControl("",[Validators.required,CustomValidators.notOnlySpaces,Validators.pattern('[0-9]{16}')]),
        securityCode: new FormControl("",[Validators.required,CustomValidators.notOnlySpaces,Validators.pattern('[0-9]{3}')]),
    ...
    this.formService.getMonthsData(startMonth).subscribe(data => this.creditCardMonths = data)
    this.formService.getYearsData().subscribe(data => this.creditCardYears = data)
  ...
  handleShowedMonths() {
    ...
    const selectedYear:number = Number(this.checkoutFormGroup.get('creditCard')!.get('expirationYear')!.getRawValue())
